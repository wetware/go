...
func main() {
	...
	fmt.Println("  ww run --with-fd input=3 3<test.txt /tmp/fd-demo")
	fmt.Println()
	fmt.Println("Example 2: Multiple file descriptors")
	fmt.Println("  echo 'data' > input.txt")
	fmt.Println("  touch output.txt")
	fmt.Println("  ww run --with-fd input=3 --with-fd output=4 3<input.txt 4>output.txt /tmp/fd-demo")
	fmt.Println()
	fmt.Println("Example 3: Database and cache fds")
	fmt.Println("  ww run --with-fd db=3 --with-fd cache=4 3<db.txt 4<cache.txt /tmp/fd-demo")
	fmt.Println()
